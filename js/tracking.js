...
function renderTimeline(historial = []) {
  ...
  const historialOrdenado = [...historial].sort((a, b) => (b?.fecha?.toMillis?.() || 0) - (a?.fecha?.toMillis?.() || 0));
  timelineEl.innerHTML = historialOrdenado.map((item) => `
...
window.buscarEnvio = async function () {
  ...
  if (!codigo) {
    await showInfo("Ingresa un código de envío.");
    return;
  }
